...
import type { Character, CharacterModule, Skill, Hindrance, AncestralAbility, Edge, Advancement, EquipmentItem, Power, Weapon } from '../domain/rpg'
import { ReferenceSidebar } from '../components/ReferenceSidebar'

function DieShape({ sides, active, onClick }: { sides: 4 | 6 | 8 | 10 | 12, active: boolean, onClick: () => void }) {
  const size = 24
...
function DamageTrack({ 
  ...
  return (
    <div className="damage-track">
      ...
      <div className="damage-track__boxes" style={{ flexDirection: reverse ? 'row-reverse' : 'row' }}>
        {Array.from({ length: max }).map((_, i) => {
          ...
          return (
            <div 
              ...
              onClick={() => onChange(isActive && value === step ? step - 1 : step)}
            >
...
type ModuleProps = {
  ...
  actions: {
    ...
    updateCharacterAttributes: (id: string, attrs: Character['attributes']) => void
    updateCharacterModule: (charId: string, moduleId: string, updates: Partial<CharacterModule>) => void
  }
}
...
function ModuleActions({ 
  ...
  const toggleSpan = () => {
    ...
    props.actions.updateCharacterModule(props.character.id, module.id, { span: nextSpan })
  }

  const currentRowSpan = module.rowSpan || 1
  const toggleRowSpan = () => {
    const nextRowSpan = currentRowSpan >= 3 ? 1 : (currentRowSpan + 1) as 1 | 2 | 3
    props.actions.updateCharacterModule(props.character.id, module.id, { rowSpan: nextRowSpan })
  }

  return (
    <div className="module-actions">
      {props.canMoveLeft && (
        <button className="module-action-btn" onClick={props.onMoveLeft} title="Mover para esquerda">
          <IconChevronLeft size={14} />
        </button>
      )}
      <button className="module-action-btn" onClick={props.onMoveUp} title="Mover para cima">
        <IconChevronUp size={14} />
      </button>
      <button className="module-action-btn" onClick={props.onMoveDown} title="Mover para baixo">
        <IconChevronDown size={14} />
      </button>
      {props.canMoveRight && (
        <button className="module-action-btn" onClick={props.onMoveRight} title="Mover para direita">
      ...
      >
        <IconGrid size={14} style={{ opacity: currentSpan > 1 ? 1 : 0.5, color: currentSpan > 1 ? 'var(--accent)' : 'inherit' }} />
      </button>
      ...
      >
        <IconGrid size={14} style={{ transform: 'rotate(90deg)', opacity: currentRowSpan > 1 ? 1 : 0.5, color: currentRowSpan > 1 ? 'var(--accent)' : 'inherit' }} />
      </button>
      ...
      <button className="module-action-btn module-action-btn--danger" onClick={props.onRemove} title="Remover módulo">
        <IconTrash size={14} />
      </button>
    </div>
  )
...
function ModuleCombatStats(props: ModuleProps) {
  ...
  const stats = character.stats || { ca: 10, hp: { current: 10, max: 10 }, initiative: 0 }
  ...
  function updateHp(current: number) {
    updateStats({ hp: { ...stats.hp, current: Math.min(Math.max(0, current), stats.hp.max) } })
  }

  const isSavage = character.system === 'savage_pathfinder' || module.system === 'savage_pathfinder'
    ...
    <div className="card" style={{ position: 'relative', ...gridColumnStyle }}>
      ...
      {isNotesOpen && (
        <div className="module-notes">
          <textarea
            ...
            value={module.notes || ''}
            onChange={(e) => actions.updateCharacterModule(character.id, moduleId, { notes: e.target.value })}
            rows={3}
            autoFocus
          />
        </div>
      ...
      <div className="section-header">
        <div className="section-title">
          <IconShield size={18} style={{ marginRight: 8, color: 'var(--accent)' }} />
          COMBATE
        </div>
      </div>
        ...
        <div className="edit-panel">
            ...
            <>
              <div className="field">
                ...
                <input 
                  ...
                  className="input" 
                  value={stats.pace ?? 6} 
                  onChange={(e) => updateStats({ pace: Number(e.target.value) })}
                />
              </div>
              <div className="field">
                ...
                <input 
                  ...
                  className="input" 
                  value={stats.parry ?? 2} 
                  onChange={(e) => updateStats({ parry: Number(e.target.value) })}
                />
              </div>
              <div className="field">
                ...
                <input 
                  ...
                  onChange={(e) => updateStats({ toughness: Number(e.target.value) })}
                />
              </div>
            </>
          ) : (
            <>
              ...
              <div className="field">
                ...
                <input 
                  ...
                  onChange={(e) => updateStats({ initiative: Number(e.target.value) })}
                />
              </div>
            </>
          )}
          <div className="field">
            ...
            <input 
              ...
              className="input" 
              value={stats.hp.max} 
              onChange={(e) => updateStats({ hp: { ...stats.hp, max: Number(e.target.value) } })}
            />
          </div>
        </div>
      )}
      ...
      <div className="combat-stats">
        {isSavage ? (
          <>
            ...
            <div className="stat-box">
              <IconShield className="stat-icon" style={{ color: 'var(--accent-2)' }} />
          ...
          <>
            ...
            <div className="stat-box">
              <IconHeart className="stat-icon" style={{ color: 'var(--danger)' }} />
              <div className="stat-label">PV</div>
              <div className="stat-value">
                {stats.hp.current}<span className="stat-max">/{stats.hp.max}</span>
              </div>
            </div>
            <div className="stat-box">
              <IconSword className="stat-icon" style={{ color: 'var(--accent-2)' }} />
        ...
        {isSavage ? (
          <div className="savage-tracks">
            ...
            <div 
              ...
              onClick={() => updateStats({ isIncapacitated: !stats.isIncapacitated })}
            >
          ...
          <>
            <div className="hp-labels">
              ...
              <span>{stats.hp.current}/{stats.hp.max}</span>
            </div>
            ...
            <div className="hp-controls">
              <button className="hp-btn" onClick={() => updateHp(stats.hp.current - 1)}>-</button>
              <button className="hp-btn" onClick={() => updateHp(stats.hp.current + 1)}>+</button>
            </div>
          </>
        )}
...
function ModuleAttributes(props: ModuleProps) {
  const { character, actions, moduleId } = props
  const module = character.modules.find((m) => m.id === moduleId)
  const attrs = character.attributes || { agility: 4, smarts: 4, spirit: 4, strength: 4, vigor: 4 }
  const [isNotesOpen, setIsNotesOpen] = useState(false)
            ...
            <div className="attribute-dice">
              {[4, 6, 8, 10, 12].map((d) => (
                <DieShape 
                  ...
                  active={attr.value === d} 
                  onClick={() => actions.updateCharacterAttributes(character.id, { ...attrs, [attr.key]: d })}
                />
              ))}
            </div>
...
function ModuleSkills(props: ModuleProps) {
  ...
  return (
    <div className="card" style={{ position: 'relative', ...gridColumnStyle }}>
      ...
      <div className="section-header">
        <div className="section-title">
          <IconScroll size={18} style={{ marginRight: 8, color: 'var(--accent)' }} />
          PERÍCIAS
        </div>
      </div>
      ...
      <div className="skills-list">
            ...
            <div className="skill-row">
              ...
              <div className="skill-actions">
                <button 
                  ...
                  onClick={() => setOpenSkillNoteId(openSkillNoteId === skill.id ? null : skill.id)}
        ...
        <button className="button button--ghost" onClick={addSkill} style={{ marginTop: 8, width: '100%', justifyContent: 'center' }}>
...
function ModuleHindrances(props: ModuleProps) {
  ...
  const module = character.modules.find((m) => m.id === moduleId)
  const [isNotesOpen, setIsNotesOpen] = useState(false)
  const [openHindranceNoteId, setOpenHindranceNoteId] = useState<string | null>(null)
  ...
  useEffect(() => {
    if (module.type === 'text_block') {
        ...
        if (text.trim()) {
            const newHindrances = text.split('\n').filter(t => t.trim()).map(line => ({
                id: crypto.randomUUID(),
                name: line.trim()
            }));
            actions.updateCharacterModule(character.id, moduleId, {
  ...
  function updateHindrance(id: string, updates: Partial<Hindrance>) {
    updateHindrances(hindrances.map(h => h.id === id ? { ...h, ...updates } : h))
  }
  ...
  return (
    <div className="card" style={{ position: 'relative', ...gridColumnStyle }}>
      ...
      <div className="section-header">
        <div className="section-title">
          <IconSkull size={18} style={{ marginRight: 8, color: 'var(--accent)' }} />
          COMPLICAÇÕES
        </div>
      </div>
      ...
      <div className="skills-list">
          ...
          <div key={hindrance.id} className="skill-row-container">
            <div className="skill-row">
              <input 
                ...
                onChange={(e) => updateHindrance(hindrance.id, { name: e.target.value })}
                style={{ paddingLeft: 8 }}
              />
              <div className="skill-actions">
                <button 
                  className={`skill-action-btn ${hindrance.notes ? 'skill-action-btn--active' : ''}`}
                  onClick={() => setOpenHindranceNoteId(openHindranceNoteId === hindrance.id ? null : hindrance.id)}
                  title="Anotação"
                >
                  <IconEdit size={16} style={{ display: 'block' }} />
                </button>
            ...
            {openHindranceNoteId === hindrance.id && (
              <div className="skill-note">
                <textarea
                  ...
                  onChange={(e) => updateHindrance(hindrance.id, { notes: e.target.value })}
                  rows={2}
                  autoFocus
                />
              </div>
        ...
        <button className="button button--ghost" onClick={addHindrance} style={{ marginTop: 8, width: '100%', justifyContent: 'center' }}>
          <IconPlus size={14} style={{ marginRight: 6 }} /> Adicionar Complicação
        </button>
      </div>
    </div>
  )
...
function ModuleAncestralAbilities(props: ModuleProps) {
  ...
  const module = character.modules.find((m) => m.id === moduleId)
  const [isNotesOpen, setIsNotesOpen] = useState(false)
  const [openAbilityNoteId, setOpenAbilityNoteId] = useState<string | null>(null)
  ...
  // Helper para garantir tipagem dos dados
  const abilities = (module.data?.ancestral_abilities as AncestralAbility[]) || []
  ...
  function updateAbility(id: string, updates: Partial<AncestralAbility>) {
    updateAbilities(abilities.map(a => a.id === id ? { ...a, ...updates } : a))
  }
  ...
  return (
    <div className="card" style={{ position: 'relative', ...gridColumnStyle }}>
      ...
      <div className="section-header">
        <div className="section-title">
          <IconSparkles size={18} style={{ marginRight: 8, color: 'var(--accent)' }} />
      ...
      <div className="skills-list">
            ...
            <div className="skill-row">
              ...
              <div className="skill-actions">
                <button 
                  ...
                  onClick={() => setOpenAbilityNoteId(openAbilityNoteId === ability.id ? null : ability.id)}
                  title="Anotação"
                >
                  <IconEdit size={16} style={{ display: 'block' }} />
                </button>
            ...
            {openAbilityNoteId === ability.id && (
              <div className="skill-note">
                <textarea
                  ...
                  value={ability.notes || ''}
                  onChange={(e) => updateAbility(ability.id, { notes: e.target.value })}
                  rows={2}
                  autoFocus
                />
              </div>
        ...
        <button className="button button--ghost" onClick={addAbility} style={{ marginTop: 8, width: '100%', justifyContent: 'center' }}>
...
function ModuleEdgesAdvancements(props: ModuleProps) {
  ...
  const [openAdvancementNoteId, setOpenAdvancementNoteId] = useState<string | null>(null)
  ...
  const advancements = (module.data?.advancements as Record<string, Advancement>) || {}
  ...
  function updateAdvancement(slotId: string, value: string, notes?: string) {
    const current = advancements[slotId] || { id: slotId, value: '', notes: '' }
    const next = { ...current, value, ...(notes !== undefined ? { notes } : {}) }
  ...
  return (
    <div className="card" style={{ position: 'relative', ...gridColumnStyle }}>
      ...
      <div className="section-header">
        <div className="section-title">
          <IconStar size={18} style={{ marginRight: 8, color: 'var(--accent)' }} />
      ...
      <div className="skills-list" style={{ marginBottom: 24 }}>
            ...
            <div className="skill-row">
              ...
              <div className="skill-actions">
                <button 
                  ...
                  onClick={() => setOpenEdgeNoteId(openEdgeNoteId === edge.id ? null : edge.id)}
                  title="Anotação"
                >
                  <IconEdit size={16} style={{ display: 'block' }} />
                </button>
        ...
        <button className="button button--ghost" onClick={addEdge} style={{ marginTop: 8, width: '100%', justifyContent: 'center' }}>
              ...
              <div className="advancement-row">
                ...
                <div className="skill-actions">
                  <button 
                    ...
                    onClick={() => setOpenAdvancementNoteId(openAdvancementNoteId === slot.id ? null : slot.id)}
                    title="Anotação"
                  >
                    <IconEdit size={16} style={{ display: 'block' }} />
                  </button>
              ...
              {openAdvancementNoteId === slot.id && (
                <div className="skill-note">
                  <textarea
                    ...
                    value={data.notes || ''}
                    onChange={(e) => updateAdvancement(slot.id, data.value, e.target.value)}
                    rows={2}
                    autoFocus
                  />
                </div>
...
function ModuleEquipment(props: ModuleProps) {
  ...
  return (
    <div className="card" style={{ position: 'relative', ...gridColumnStyle }}>
      ...
      <div className="section-header">
        <div className="section-title">
          <IconBackpack size={18} style={{ marginRight: 8, color: 'var(--accent)' }} />
          EQUIPAMENTO
        </div>
      </div>
      ...
      <div className="equipment-base-gold">
        ...
        <div className="input-wrapper" style={{ display: 'flex', alignItems: 'center', background: 'rgba(0,0,0,0.2)', padding: '4px 8px', borderRadius: 4, border: '1px solid var(--border)' }}>
            <IconCoin size={14} style={{ marginRight: 6, color: 'var(--accent)' }} />
            <input 
                type="number"
                className="input"
                style={{ border: 'none', padding: 0, background: 'transparent', width: 80 }}
                value={baseGold || ''}
      ...
      <div className="equipment-list-header" style={{ display: 'flex', padding: '0 8px 4px', fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: 0.5 }}>
      ...
      <div className="skills-list">
        {items.map((item) => (
          <div key={item.id} className="equipment-row-container" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
            <div className="equipment-row" style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '4px 8px' }}>
              <input 
              ...
              <div className="skill-actions">
                  <button 
                    ...
                    onClick={() => setOpenItemNoteId(openItemNoteId === item.id ? null : item.id)}
                    title="Anotação"
                  >
                    <IconEdit size={16} style={{ display: 'block' }} />
                  </button>
        ...
        <button className="button button--ghost" onClick={addItem} style={{ marginTop: 8, width: '100%', justifyContent: 'center' }}>
          <IconPlus size={14} style={{ marginRight: 6 }} /> Adicionar Item
        </button>
      </div>

      <div className="equipment-footer" style={{ marginTop: 16, borderTop: '1px dashed var(--border)', paddingTop: 12, display: 'flex', justifyContent: 'space-between' }}>
        <div className="equipment-stat" style={{ display: 'flex', flexDirection: 'column' }}>
            <span className="label" style={{ fontSize: 11, color: 'var(--muted)' }}>PESO TOTAL</span>
            <span className="value" style={{ fontSize: 16, fontWeight: 'bold' }}>{totalWeight} kg</span>
        </div>
        <div className="equipment-stat" style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
            <span className="label" style={{ fontSize: 11, color: 'var(--muted)' }}>OURO RESTANTE</span>
            <span className="value" style={{ fontSize: 16, fontWeight: 'bold', color: currentGold < 0 ? 'var(--danger)' : 'var(--accent)' }}>
                {currentGold}
            </span>
        </div>
      </div>
...
function ModulePowerPoints(props: ModuleProps) {
  ...
  return (
    <div className="card" style={cardStyle}>
      ...
      <div className="section-header">
        <div className="section-title">
          <IconBolt size={18} style={{ marginRight: 8, color: isLow ? 'var(--danger)' : 'var(--accent)' }} />
          PONTOS DE PODER
          <button
            className="button button--icon"
            title="Resetar para o Máximo"
            onClick={() => updateData({ current: max })}
            style={{ width: 24, height: 24, padding: 0, opacity: 0.6, marginLeft: 8 }}
          >
            <IconRefresh size={14} />
          </button>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '8px 0 16px 0' }}>
        ...
        <div style={{ display: 'flex', gap: 16, alignItems: 'center', width: '100%', justifyContent: 'center' }}>
            <button 
                className="button"
                style={{ width: 48, height: 48, borderRadius: '50%', padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, fontWeight: 'bold' }}
        ...
        {/* Configuração de Max */}
        <div style={{ marginTop: 24, display: 'flex', alignItems: 'center', gap: 8, opacity: 0.6, fontSize: 12 }}>
            <span style={{ color: 'var(--muted)' }}>MÁXIMO:</span>
            <input 
                type="number" 
                className="input" 
                style={{ width: 60, textAlign: 'center', padding: '2px 4px', background: 'rgba(0,0,0,0.2)', border: '1px solid var(--border)' }}
                value={max}
...
function ModulePowers(props: ModuleProps) {
  ...
  return (
    <div className="card" style={{ position: 'relative', ...gridColumnStyle }}>
      ...
      <div className="section-header">
        <div className="section-title">
          <IconBolt size={18} style={{ marginRight: 8, color: 'var(--accent)' }} />
          PODERES
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 14 }}>
        <div style={{ fontSize: 12, color: 'var(--muted)' }}>
          {ppCurrent === null || ppMax === null ? 'Pontos de Poder: (adicione o módulo Pontos de Poder)' : `Pontos de Poder: ${ppCurrent} / ${ppMax}`}
        </div>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {powers.map((p) => (
          <div key={p.id} style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, background: 'rgba(0,0,0,0.12)' }}>
            <div style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '10px 10px 8px 10px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  <button
                    ...
                    title={!ppModule ? 'Módulo PP não encontrado' : `Gastar ${p.powerPoints || 0} PP`}
                    style={{ height: '100%', width: 28, justifyContent: 'center', opacity: (!ppModule || !p.powerPoints) ? 0.3 : 1 }}
                  >
                    <IconMagic size={14} />
                  </button>
              </div>
              ...
              <input
                ...
                onChange={(e) => updatePower(p.id, { powerPoints: Number(e.target.value) })}
              ...
              <input
                ...
                placeholder="Duração"
                value={p.duration}
                onChange={(e) => updatePower(p.id, { duration: e.target.value })}
                style={{ flex: 1.2 }}
              />
              <button
                className="skill-action-btn skill-action-btn--danger"
      ...
      <button className="button button--ghost" onClick={addPower} style={{ marginTop: 12, width: '100%', justifyContent: 'center' }}>
...
function ModuleWeapons(props: ModuleProps) {
  ...
  return (
    <div className="card" style={{ position: 'relative', ...gridColumnStyle }}>
      ...
      <div className="section-header">
        <div className="section-title">
          <IconSword size={18} style={{ marginRight: 8, color: 'var(--accent)' }} />
          ARMAS
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {weapons.map((w) => (
          <div key={w.id} style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, background: 'rgba(0,0,0,0.12)' }}>
            <div style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '10px 10px 8px 10px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              <input
                className="input"
                placeholder="Arma"
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '8px 10px', background: 'rgba(0,0,0,0.2)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, flex: 1 }}>
                    <span style={{ fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase' }}>PA:</span>
                    <input
                        className="input"
                        type="number"
                        value={w.ap || 0}
                        onChange={(e) => updateWeapon(w.id, { ap: Number(e.target.value) })}
                        style={{ width: 40, textAlign: 'center', padding: '2px 4px', fontSize: 12 }}
                    />
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, flex: 1 }}>
                    <span style={{ fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase' }}>CdT:</span>
                    <input
                        className="input"
                        type="number"
                        value={w.rof || 1}
                        onChange={(e) => updateWeapon(w.id, { rof: Number(e.target.value) })}
                        style={{ width: 40, textAlign: 'center', padding: '2px 4px', fontSize: 12 }}
                    />
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, flex: 1 }}>
                    <span style={{ fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase' }}>Peso:</span>
                    <input
                        className="input"
                        type="number"
                        value={w.weight || 0}
                        onChange={(e) => updateWeapon(w.id, { weight: Number(e.target.value) })}
                        style={{ width: 40, textAlign: 'center', padding: '2px 4px', fontSize: 12 }}
                    />
                </div>
            </div>
      ...
      <button className="button button--ghost" onClick={addWeapon} style={{ marginTop: 12, width: '100%', justifyContent: 'center' }}>
...
export function CharacterSheetPage() {
  ...
  function handleAddModule(type: CharacterModule['type'], system: CharacterModule['system']) {
    if (!character) return
    const defaultLayout = type === 'edges_advancements'
      ? { span: 1 as const, rowSpan: 2 as const, column: 2 as const }
      : {}
    actions.addCharacterModule(character.id, { type, system, ...defaultLayout })
  ...
  return (
    <div className="page" style={{ maxWidth: 1200, paddingBottom: 100, position: 'relative' }}>
      <input
        type="file"
        ref={fileInputRef}
        style={{ display: 'none' }}
      ...
      <div className="char-header card" style={{ marginBottom: 24 }}>
        <div 
          ...
          style={{ backgroundImage: `url('${character.avatarUrl || 'https://placehold.co/80x80/2a2a2a/FFF?text=Avatar'}')` }}
        ...
        <div className="char-header__info">
          ...
          <div className="char-header__meta">
            ...
            {character.race || 'Raça'} {character.class || 'Classe'} • Nível {character.level || 1}
          </div>
        </div>
        ...
        <div className="modal-overlay" onClick={() => setIsStoryOpen(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h3 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: 'var(--accent)', display: 'flex', alignItems: 'center', gap: 10 }}>
                <IconScroll size={20} />
            ...
            <textarea
              className="input input--textarea"
              style={{ minHeight: 400, fontSize: 16, lineHeight: 1.6, padding: 20, resize: 'none' }}
              value={character.background || ''}
              onChange={(e) => actions.updateCharacter(character.id, { background: e.target.value })}
      ...
      <div className="modules-grid">
        {modules.map((mod) => {
          ...
          const props: ModuleProps = {
            ...
            onMoveUp: () => actions.reorderCharacterModule(character.id, mod.id, 'up'),
            onMoveDown: () => actions.reorderCharacterModule(character.id, mod.id, 'down'),
            onMoveLeft: () => actions.moveCharacterModuleColumn(character.id, mod.id, 'left'),
            onMoveRight: () => actions.moveCharacterModuleColumn(character.id, mod.id, 'right'),
            canMoveLeft: colIndex > 0,
            canMoveRight: colIndex < 2,
          }

          if (mod.type === 'combat_stats') return <ModuleCombatStats key={mod.id} {...props} />
          if (mod.type === 'attributes') return <ModuleAttributes key={mod.id} {...props} />
          if (mod.type === 'skills') return <ModuleSkills key={mod.id} {...props} />
          if (mod.type === 'hindrances' || mod.type === 'text_block') return <ModuleHindrances key={mod.id} {...props} />
          if (mod.type === 'ancestral_abilities') return <ModuleAncestralAbilities key={mod.id} {...props} />
          if (mod.type === 'edges_advancements') return <ModuleEdgesAdvancements key={mod.id} {...props} />
          if (mod.type === 'equipment') return <ModuleEquipment key={mod.id} {...props} />
          if (mod.type === 'power_points') return <ModulePowerPoints key={mod.id} {...props} />
          if (mod.type === 'powers') return <ModulePowers key={mod.id} {...props} />
          if (mod.type === 'weapons') return <ModuleWeapons key={mod.id} {...props} />
          return null
        })}
      </div>
      ...
      <div className="floating-dock">
            ...
            <div className="add-menu">
              <div className="add-menu__section">
                ...
                <button 
                  ...
                  onClick={() => handleAddModule('combat_stats', 'savage_pathfinder')}
                >
                ...
                <button 
                  className="add-menu__item"
                  onClick={() => handleAddModule('attributes', 'savage_pathfinder')}
                >
                ...
                <button 
                  className="add-menu__item" 
                  onClick={() => handleAddModule('hindrances', 'savage_pathfinder')}
                >
                ...
                <button 
                  className="add-menu__item" 
                  onClick={() => handleAddModule('ancestral_abilities', 'savage_pathfinder')}
                >
                ...
                <button 
                  className="add-menu__item" 
                  onClick={() => handleAddModule('edges_advancements', 'savage_pathfinder')}
                >
                ...
                <button 
                  className="add-menu__item" 
                  onClick={() => handleAddModule('equipment', 'savage_pathfinder')}
                >
                ...
                <button 
                  className="add-menu__item" 
                  onClick={() => handleAddModule('power_points', 'savage_pathfinder')}
                >
                ...
                <button 
                  className="add-menu__item" 
                  onClick={() => handleAddModule('weapons', 'savage_pathfinder')}
                >
              ...
              <div className="add-menu__section">
                ...
                <button className="add-menu__item" disabled style={{ opacity: 0.5 }}>
        ...
        <button className="dock-btn dock-btn--danger" onClick={() => navigate('/characters')}>
          Fechar Ficha
        </button>
      </div>
